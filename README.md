...
| [`census/human/`](census/human) — human-rater pass H1 (NOT a census; stratified sample, no share headline, no number joins any series) | 60 | REGISTERED, not yet run — frozen 60-item sample (20 outside-set soft residue + 11 recognition/J4 + 29 seeded J5 draws), frozen v6 pack served verbatim inside the packet (byte-identity machine-checked); clauses H1 ≥25/29 DOMAIN + H2 ≥6/9 PROCESS, both floors the smallest integer every measured fail branch fails; match-vs-v6 report-only beside the measured non-anchor band 50–59/60 | registration `0cf32a7` pushed before any rater rates; the probe of limitation 6's corpus-shared-prior confound — the sole remaining route after the obfuscation probe; rater seat: a recruited non-author human, blind, identity via marked append-only addendum before rating; awaiting the recruit |
