...
| [`census/obfuscation/`](census/obfuscation) — obfuscation probe of limitation 6's memorization component (NOT a census; no number joins any series) | 225 | **O1 manipulation check FAILS both arms — the corpus is identified as RFC 5545 from STRUCTURE and disclosed residuals (95%/98% confidence) *[corrected 2026-08-20: "structure alone" overstated it — the obfuscation registration discloses retained lexical residuals (sibling-RFC citations, "rule part(s)", the fold vocabulary) and its finding 1 names a lexical route in the identifying reasoning]* through 90 nonced identifiers and 8 nonced domain nouns** — while O2–O4 pass perfectly under the pre-committed downgrade: match-vs-v6 224/225 in both raters, pair 222/225 = 98.7% (exact tie with the repo record), DOMAIN mass 194/194 ×2, zero measurable nonce cost | run 2026-08-16; registration `c500361` pushed before any agent (seeded reversible map + script shipped; four caught leak classes disclosed); the check's failure IS the finding: lexical obfuscation cannot isolate the memorization channel, so no LLM-side design reaches the shared-prior confound — limitation 6's only-a-non-LLM-rater claim, previously an argument, is now a measurement; the human-rater replication remains the only probe that closes it |
| [`census/mech-probe/`](census/mech-probe) — MECH-PROBE-1, feasibility probe of a non-LLM rule-based classifier (NOT a census; no number joins any series) | 127 | **INCONCLUSIVE — not pursued.** Whole corpus: classifier 26.8% (a generous reading — it scores the classifier's abstentions as correct `U` calls; scoring them as misses gives 20.5%/23.6%) — **below** the constant degenerate (30.7%/33.1%) but **above** the shuffle by only ~5.5 points (21.4% — the 2000-seed mean; an earlier version of this row quoted 12.6%, a single declared seed sitting in the bottom half-percentile). `PLAN.md` step 5 requires beating both, and its phrase "the SAME items" is ambiguous between the covered 56 and all 127 — **the pre-registration cannot adjudicate this run**; see C8.2. Covered items only (44.1% coverage): 46.4%/53.6%, beating both degenerates by 17.9–32.1 points. Two rounds of cold-review corrections withdraw the original causal claim AND the original "arc closed" verdict. ⚠️ **No registration was pushed before this run** — all files landed in one commit; do not read it as meeting this repo's pre-registration standard | MLS spent by this run; TLS/iCal/QUIC protected. Grader ships with a KAT |
| [`census/human/`](census/human) — human-rater pass H1 (NOT a census; stratified sample, no share headline, no number joins any series) | 60 | ⛔ **H1 SUPERSEDED 2026-08-20 by H1-R2** — `packet-h1.md` leaked real archive labels in its format examples; one leak converts the largest measured FAILING branch into a PASS at the H2 floor. **Serve `packet-h1r2.md`, never `packet-h1.md`.** H1-R2 REGISTERED, not yet run, awaiting a blind recruit; sample/clauses/floors unchanged and re-verified | defect disclosed + measured at the artifact; superseding registration and cold-review corrections in the same README |
...
| [`census/v7-pass/`](census/v7-pass) — first v7 pass over iCal/TLS/QUIC (NOT new corpora, FIFTH instrument series) | 710 | v7 shares: iCal 88.4%/87.6%, TLS 80.9%/80.9%, QUIC 63.0%/63.3% — every one at or below its v6 counterpart; pair agreements 97.8% / 92.2% / 86.5%, all inside their corpus bands, no records | run 2026-08-16; amendment `70dddbe` (rules 26/27 at the v6 pass's measured edges) and registration `3c1f090` pushed before any rater; **both adjudications transmit cross-family** (150 PROCESS, 62 DOMAIN, 64/232 PROCESS — eight readings each way since the wall; recognition nine 18/18), TLS posts the series' FIRST fully clean sheet and the CV core reaches seventeen consecutive raters across six families — while the tripwires convict the author's calibration three ways: L6 fails in the foreign seat (191/194), iCal 23(b) fails ×2 at the zero bound (the perennial U-flickers 77/79), QUIC 23(b) fails ×2 (11/15 vs 7; the foreign seat breaks the history-quartet's TYPESTATE streak) |
| [`census/locality/`](census/locality) — locality study: is the transmissibility boundary a semantic property? (NOT a census; no raters; no number joins any series) | 44 | headline correspondence (local rung witnessed ⟺ eliminable vote majority) holds with exactly the four pre-named exceptions {75, 111, 164, 188}; contested single-rung predictions 16/18, both misses one shape (a hidden context dependency); CV splits three ways under the lens — the secret is the discriminator, witnessed | run 2026-08-16; registration `20b62e9` (formal criterion, 44-item set from all fourteen archived TLS maps, grader) pushed before any witness; witnesses are constructive proofs — validators for locality, distinguishing pairs for non-locality — author-constructed and archive-aware (disclosed; the artifacts, not the pass-rate, are the product); the gate's reviewer REFUTED one recorded FAILS entry (pre-push) by constructing the pair it denied — the reader-challenge mechanism firing on its first outing |
| [`census/locality2/`](census/locality2) — second locality passes over iCalendar and QUIC (NOT a census; no raters; no number joins any series) | 23 + 49 | the correspondence generalizes with class-shaped, pre-named divergence: all 22 named exceptions realized, zero unnamed (three corpora: 26/26, zero unnamed, 116 items); QUIC's conn-carried THRESHOLD family (the 13-item limit cluster + 3 unanimous samples) is conn-local yet off the eliminable side, and TYPESTATE-unanimous item 58 came out EMPTY as predicted (remembered 0-RTT values live in a prior connection); iCalendar's {62, 77, 79} = rule 25's trigger/duty split witnessed; byte-identical items 43/79 straddle the vote-majority line, so the mismatch had to land on one — it landed where registered | run 2026-08-17; registration `8821bce` (per-genre granularities, 72-item sets from all ten + twelve archived maps, 37 KATs, FAILS-coverage-enforcing grader) pushed before any witness; 71/72 exact outcomes; the gate's reviewer REFUTED the 235-pkt FAILS record (pre-push) by constructing the validator it denied — the challenge interface's second firing in two studies |
| [`census/foreign2/`](census/foreign2) — second cross-family replication: the panel widens to five foreign families (NOT a new corpus) | 204 | quotients Gemini 78.9% / Kimi 78.9% / GLM 77.0% — all inside the pre-registered 76–86% band, four of five foreign quotients below the lowest Claude quotient; CV core **15-for-15 across six families**, K's penumbra exactly G's ∪ M's; ten foreign–foreign pairs at 79.9–88.2% raw | run 2026-08-16; registration `7d5d5a8` pushed before any of the three models saw instrument or corpus (enabled by a measured change: the first replication's recorded flash-tier-only Gemini exclusion no longer holds); F6–F9 PASS ×3, **F10 FAILS in Kimi** — the candidate-rule-17 negotiated-state cluster is not seat-general: Kimi sides with rater D's archived DOMAIN reading (8/9) while Gemini/GLM side with the G=X TYPESTATE consensus (7/9 each), and D is the Anthropic archive's own outlier — the rule-17 docket gains the split's structure |
...
9. ~~A NON-protocol corpus~~ — DONE 2026-08-15 (`census/ical/`): RFC
   ...
   2026-08-16, PAPER.md §6.3 ("The verdict answered"); the
   OBFUSCATION PROBE ran 2026-08-16 (`census/obfuscation/`):
   ...
   measurement and the appendix its row); the HUMAN-RATER PASS H1 is
   ⛔ SUPERSEDED (2026-08-20) by **H1-R2** — `packet-h1.md` leaked real
   archive labels in its format examples and one leak lifts the largest
   measured failing branch to a PASS at the H2 floor; **serve
   `packet-h1r2.md`, never `packet-h1.md`**. H1-R2 is REGISTERED
   (`census/human/` — sample, clauses and floors unchanged and
   re-verified; grader carries a negative-controlled answer-leak guard;
   awaiting a recruited non-author human, ~90 minutes); MECH-PROBE-1 is
   DONE and INCONCLUSIVE (2026-08-20, `census/mech-probe/` — a
   rule-based non-LLM classifier, the one instrument needing no recruit;
   below the constant degenerate on the whole corpus but ~5.5 points
   above the seed-distribution shuffle mean, and PLAN.md's phrase "the
   SAME items" is ambiguous, so the pre-registration cannot adjudicate
   which branch fires;
   not pursued, and MLS is spent as a probe corpus); the
   SECOND CROSS-FAMILY REPLICATION is DONE (2026-08-16,
   ...
   LOCALITY STUDY is DONE (2026-08-16, `census/locality/` — rule 16's
   litmus formalized and tested against all fourteen archived TLS
   maps by constructive witnesses; the four named exceptions realized
   exactly, two prediction failures of one shape, one FAILS record
   refuted at the gate; PAPER §6.6 integrated); the SECOND LOCALITY
   PASSES are DONE (2026-08-17, `census/locality2/` — the same
   criterion over the iCalendar and QUIC archives, per-genre
