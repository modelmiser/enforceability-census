...
| [`census/mech-probe/`](census/mech-probe) — MECH-PROBE-1, feasibility probe of a non-LLM rule-based classifier (NOT a census; no number joins any series) | 127 | **INCONCLUSIVE — not pursued.** Whole corpus: classifier 26.8% — **below** the constant degenerate (30.7%/33.1%) but **above** the shuffle (12.6%); `PLAN.md` requires beating both, so branch 1 is ruled out and branches 2/3 are not separated. Covered items only (44.1% coverage): 46.4%/53.6%, beating both degenerates by 17.9–32.1 points. Two rounds of cold-review corrections withdraw the original causal claim AND the original "arc closed" verdict. ⚠️ **No registration was pushed before this run** — all files landed in one commit; do not read it as meeting this repo's pre-registration standard | MLS spent by this run; TLS/iCal/QUIC protected. Grader ships with a KAT |
...
9. ~~A NON-protocol corpus~~ — DONE 2026-08-15 (`census/ical/`): RFC
   ...
   below the constant degenerate on the whole corpus but above the
   shuffle, so branch 1 is ruled out and branches 2/3 are not separated;
